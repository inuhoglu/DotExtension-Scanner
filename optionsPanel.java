...
public class optionsPanel extends Observable{
    ...
    public optionsPanel() {
    	...
    	extensionPanel.add(new JLabel("The file extension "));
    	...
    	keyPanel.add(new JLabel("The search parameter "));
    	...
    	public void actionPerformed(ActionEvent event){
    		if(event.getSource()==fileChoose){
    			...
    			for(int i=0;i<fH.getFileNames().size();i++){
    				try{
    					scan=new Scanner(new File((String)(fH.getFileNames().get(i))));	//create file scanner to scan inside
    				}
    				catch(IOException except){	//continue process in case of error
    ...
    private class RBListener implements ActionListener{
    	public void actionPerformed(ActionEvent event){	//Listen radio buttons
